function effect_toe(vehicle_data,Ts,Tf)
    ...
    for i = 1:5
        ...
        fprintf('End simulation #%.1f /5\n',i);
    end
    
    colorMap = colormap(lines(5));
    figure('Name','Normalized lateral forces for diff toe angles','NumberTitle','off'), clf
    hold on
    for i = 1:5
        %plot(alphaR_dt(:,i),Fyr_dt_norm(:,i),'LineWidth',2, 'DisplayName',("Yr(ay) with toe angle $\delta_0$ =  " + toe_vec(1,i)), 'Color', colorMap(i,:));
        plot(alphaF_dt(:,i),Fyf_dt_norm(:,i),'LineWidth',2, 'DisplayName',("Yf(ay) with toe angle $\delta_0$ =  " + toe_vec(1,i)), 'Color', colorMap(i,:));
    ...
    figure('Name','Handling diagram for diff toe angles','NumberTitle','off'), clf
    hold on
    for i = 1:5
        plot(Ay(:,i)./g, -delta_alpha_dt(2:end,i), 'LineWidth',2, 'DisplayName',("$-\Delta\alpha(ay)$ with $\delta_0$ =  " + toe_vec(1,i)), 'Color', colorMap(i,:));
